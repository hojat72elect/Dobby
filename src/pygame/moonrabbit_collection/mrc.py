...
class Game(pp.PygpenGame):
    def load(self):
        ...
        for i in range(40):
            ...
            if img == self.e['Assets'].images['background']['moon']:
                img = random.choice(list(self.e['Assets'].images['background'].values()))
            self.background_items.append(([random.random() * self.display.get_width(),
                                           random.random() * self.display.get_height()], img, random.random()))

        self.fireflies = []
        for i in range(20):
            self.fireflies.append(
                [[random.random() * self.display.get_width(), random.random() * self.display.get_height()],
                 random.random() * math.pi * 2, random.random() * 10 + 10,
    ...
    def render_background(self):
        for item in self.background_items:
            self.e['Renderer'].blit(item[1], ((item[0][0] - self.camera[0] * item[2] * 0.1) % self.display.get_width(),
                                              (item[0][1] - self.camera[1] * item[
                                                  2] * 0.1) % self.display.get_height()), z=-100)
        ...
        for firefly in self.fireflies:
            ...
            wpos = (int((firefly[0][0] - self.camera[0] * 1.2) % self.display.get_width()),
                    int((firefly[0][1] - self.camera[1] * 1.2) % self.display.get_height()))
            ...
            self.e['Renderer'].blit(glow_img,
                                    (wpos[0] - glow_img.get_width() // 2, wpos[1] - glow_img.get_height() // 2), z=107)
    ...
    def update(self):
        ...
        self.tilemap.renderz(pygame.Rect(self.camera[0] - 14, self.camera[1] - 14, self.display.get_width() + 14,
                                         self.display.get_height() + 14), offset=self.camera)
        ...
        self.e['Renderer'].renderf(self.gm.update_render, self.e['Window'].dt, offset=self.camera,
                                   rot_function=lambda x, y: int(
                                       (math.sin(x / 100 + time.time() * 1.5) - 0.7) * 30) / 10, z=-5)
