    ...
    def update(self, *args, **kwargs):
        ...
        if self.digging:
            if self.e['Window'].dt > random.random() * 0.1:
                ...
                particle = pp.vfx.Particle((self.center[0], self.center[1] + 4), 'particle', decay_rate=0.25,
                                           advance=random.random() * 0.3 + 0.3,
                                           velocity=[math.cos(angle) * force, math.sin(angle) * force], behavior='dirt',
                                           colors={(255, 255, 255): (135, 53, 85)},
                                           physics_source=self.e['Game'].tilemap)
