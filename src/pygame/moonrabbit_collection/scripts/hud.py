...
class HUD(pp.ElementSingleton):
    ...
    def render(self):
        ...
        if len(self.inventory):
            self.e['Renderer'].blit(self.e['Assets'].images['items'][self.inventory[-1]],
                                    (self.e['Game'].display.get_width() // 2 - 10, 230 - self.item_notification_pos),
                                    z=100, group='ui')
            self.e['Text']['small_font'].renderz(ITEM_MAP[self.inventory[-1]] + ' Acquired!', (
            self.e['Game'].display.get_width() // 2 - self.e['Text']['small_font'].width(
                ITEM_MAP[self.inventory[-1]] + ' Acquired!') // 2, 253 - self.item_notification_pos),
                                                 color=(255, 255, 255), z=101, group='ui')
            self.e['Text']['small_font'].renderz(ITEM_MAP[self.inventory[-1]] + ' Acquired!', (
            self.e['Game'].display.get_width() // 2 - self.e['Text']['small_font'].width(
                ITEM_MAP[self.inventory[-1]] + ' Acquired!') // 2 + 1, 254 - self.item_notification_pos),
                                                 color=(0, 0, 1), z=100, group='ui')
        ...
        for i, map in enumerate(self.maps):
            ...
            self.e['Renderer'].blit(self.e['Assets'].images['maps']['map_' + map], (map_x, 230 - self.maps_pos), z=100,
                                    group='ui')
        ...
        if self.tutorial:
            ...
            self.e['Game'].player.center[0] - 16 - self.e['Game'].camera[0],
            self.e['Game'].player.pos[1] - 30 - self.e['Game'].camera[1]), z=99, group='ui')
        ...
        if self.talking:
            ...
            self.e['Renderer'].blit(self.e['Assets'].images['portraits'][self.talking_target], (17, 145), z=90,
                                    group='ui')
            self.e['Renderer'].blit(self.e['Assets'].images['misc']['portrait_overlay'], (15, 143), z=91, group='ui')
            self.e['Text']['small_font'].renderz(name[:int(self.text_progress)], (70, 156), color=(0, 0, 1), z=91,
                                                 group='ui')
            self.e['Text']['small_font'].renderz(name[:int(self.text_progress)], (71, 155), color=(255, 255, 255), z=92,
                                                 group='ui')
            if len(self.talking_text):
                ...
                if shown_chars != self.last_shown_chars:
                    if self.talking_text[0][
                        min(max(0, int(self.text_progress) - 10), len(self.talking_text[0]) - 1)] in HARD_CONSONANTS:
                        self.e['Sounds'].play(self.talking_target, volume=0.5)
                    if self.talking_text[0][
                        min(max(0, int(self.text_progress) - 10), len(self.talking_text[0]) - 1)] in SOFT_CONSONANTS:
                        self.e['Sounds'].play(self.talking_target, volume=0.3)
                self.last_shown_chars = shown_chars
                self.e['Text']['small_font'].renderz(self.talking_text[0][:max(0, int(self.text_progress) - 10)],
                                                     (70, 165), color=(255, 255, 255), line_width=200, z=92, group='ui')
                self.e['Text']['small_font'].renderz(self.talking_text[0][:max(0, int(self.text_progress) - 10)],
                                                     (71, 166), color=(0, 0, 1), line_width=200, z=91, group='ui')
