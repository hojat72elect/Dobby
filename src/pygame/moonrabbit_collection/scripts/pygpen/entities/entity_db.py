        ...
        for animation in self.config['animations']:
            if animation in self.assets:
                ...
                if len(frame_i) and frame_i.isnumeric():
                    frame_names = [(int(frame_name.split('_')[-1]), '_'.join(frame_name.split('_')[:-1])) for frame_name
                                   in self.assets[animation] if frame_name.split('_')[-1].isnumeric()]
                    frame_names.sort()
                    frame_names = [frame_name[1] + ('_' if len(frame_name[1]) else '') + str(frame_name[0]) for
                                   frame_name in frame_names]
                    animation_images = [self.assets[animation][frame_name] for frame_name in frame_names if
                                        type(self.assets[animation][frame_name]) == pygame.Surface]
                else:
...
class EntityDB(ElementSingleton):
    ...
    def generate_configs(self):
        for entity in os.listdir(self.path):
            animations = []
            images = []
            config = {'images': {}, 'animations': {}, 'file_path': self.path + '/' + entity, 'id': entity,
                      'centered': False, 'offset': [0, 0], 'colorkey': [0, 0, 0],
            ...
            for animation in animations:
                if animation not in config['animations']:
                    ...
                    config['animations'][animation] = {'offset': [0, 0], 'speed': 1.0, 'loop': True, 'paused': False,
                                                       'frames': []}
                    for frame in frames:
