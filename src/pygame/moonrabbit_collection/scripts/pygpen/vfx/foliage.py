    ...
    def render_functions(self):
        ...
        def foliage_render(tile, offset=(0, 0), group='default'):
            if tile.tile_id in self.e['FoliageAssets'].foliage[tile.group]:
                ...
                tile.e['Renderer'].renderf(foliage.render, (
                tile.raw_pos[0] + tile.offset[0] - offset[0], tile.raw_pos[1] + tile.offset[1] - offset[1]),
                                           m_clock=tile.e['Window'].time * tile.e['FoliageAssets'].global_speed,
                                           z=tile.layer, group=group, seed=sum(tile.raw_pos))
                ...
                if random.random() < tile.e['Window'].dt:
                    ...
                    if not tile.map.physics_gridtile(wpos):
                        leaf = Particle(wpos, 'leaf', velocity=[-15, 15], decay_rate=0.1, advance=random.random() * 0.5,
                                        behavior='grass', z=10, colors=colors)
        ...
        for ss_id in self.e['Assets'].spritesheets:
            if 'foliage_colors' in self.e['Assets'].spritesheets[ss_id]['config']:
                ...
                for tile_id in self.e['Assets'].spritesheets[ss_id]['assets']:
                    motion_scale = self.e['Assets'].spritesheets[ss_id]['config'][tile_id][
                        'motion_scale'] if 'motion_scale' in self.e['Assets'].spritesheets[ss_id]['config'][
                        tile_id] else 1
                    self.foliage[ss_id][tile_id] = AnimatedFoliage(
                        self.e['Assets'].spritesheets[ss_id]['assets'][tile_id], colors, motion_scale=motion_scale)
...
class AnimatedFoliage:
    ...
    def find_leaf_point(self):
        while True:
            point = (
            int(random.random() * self.layers[0].get_width()), int(random.random() * self.layers[0].get_height()))
            color = self.layers[0].get_at(point)
            if list(color)[:3] != [0, 0, 0]:
                return point

    def render(self, surf, pos, m_clock=0, seed=14):
        surf.blit(pygame.transform.rotate(self.layers[0], math.sin(m_clock * 0.8 + (2.7 * seed)) * 1.2), (
        pos[0] + math.sin(m_clock * 1.7 + (2.7 * seed)) * 1.5 * self.motion_scale,
        pos[1] + math.sin(m_clock + (2.2 * seed)) * self.motion_scale))
        surf.blit(self.base_image, pos)
        for i, layer in enumerate(self.layers):
            if i != 0:
                layer_img = pygame.transform.rotate(layer, math.sin(m_clock * 1.1) * 4.5)
                offset = (
                (layer_img.get_width() - layer.get_width()) // 2, (layer_img.get_height() - layer.get_height()) // 2)
                surf.blit(layer_img, (
                pos[0] + math.sin(m_clock * (1.25 * i) + (2.7 * seed)) * 1.5 * self.motion_scale - offset[0],
                pos[1] + math.sin(m_clock * (1.25 * i) + (2.2 * seed)) * self.motion_scale - offset[1]))
            else:
                surf.blit(layer, pos)
